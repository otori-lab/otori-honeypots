...
FAKE_USER = os.environ.get("FAKE_USER", "admin")
FAKE_PASS = os.environ.get("FAKE_PASS", "admin123")

HOSTKEY_PATH = Path(os.environ.get("HOSTKEY_PATH", "hostkey_rsa"))
FAKE_HOSTNAME = os.environ.get("FAKE_HOSTNAME", "srv-prod-01")
...
class HoneypotServer(paramiko.ServerInterface):
    ...
    def check_auth_password(self, username, password):
        logger.info(f"Auth attempt from {self.client_addr} user={username} pass={password}")
        ...
        if qout is not None:
            ...
            if fout is not None:
                ...
                if is_cat_on_nonexistent and out and code == 0:
                    # LLM generated content for a non-existent file - persist it
...
def main():
    ...
    logger.info(f"SSH honeypot listening on {LISTEN_HOST}:{LISTEN_PORT}")
